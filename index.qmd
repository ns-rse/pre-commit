...
of type text. There are then checks for `docstring-first`, `case-conflict` and finally a `check-yaml` hook which makes
sure any YAML files are correctly formatted.
:::
...
checks we configured are run against **only** the files we are committing, in this case `sample.py`. There is no
downloading of the environments as there was with the first manual run and as we've corrected the linting errors the tests
pass and the commit is made.
