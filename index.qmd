...
::: {.notes}
When writing code and  its good practice to lint your code to ensure it conforms to the languages style guides. These
...
sometimes even different styles within a language. For Python there is the PEP8 style guide and there are a number of
tools that help with linting Python code to be comply with PEP8 such as `black` which mostly applies formatting,
`flake8` and `pylint` which check formatting and also look for code smells where poor design patterns have been used and
...
![](assets/img/pre-commit-buttong.png)
```{.markdown}
...
* :heavy_check_mark: Automates running test suites and that they pass.
