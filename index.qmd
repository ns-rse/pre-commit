...
::: {.notes}
This is the main pre-commit repository configuration, it defines the repository from which the hook is downloaded and you must
specify a revision to use.

A series of `hooks` are then defined, here the `trailing-whitespace` and `end-of-file-fixer` are enabled to run on files
of type text. There are then checks for `docstring-first`, `case-conflict`, `requirements-fixer`, `byte-order-marker`
and finally a `check-yaml` hook enabled.
:::
...
checks we configured are run against the files we are committing, in this case `sample.py`.
